...
class Admin extends CI_Controller
{
  ...
  public function add_product()
  {
    ...
    $this->form_validation->set_rules('ket_produk', 'Keterangan Produk', 'min_length[20]|required|max_length[200]|trim');
    ...
    if ($this->form_validation->run() == false) {
      ...
      if (!empty($_FILES['foto']['name'])) {
        $data = [
          ...
          'gambar_tambahan' => $this->uploadFotoProdukTambahan(),
          'create_date' => time()
        ];
  ...
  public function uploadFotoProdukUpdate()
  {
    $config['upload_path']       = './assets/images/produk/';
    $config['allowed_types']     = 'gif|jpg|png|jpeg';
    $config['max_size']          = 4000;   //Tergantung dari php.ini juga
  ...
  public function uploadFotoProdukTambahan()
  {
    $config['upload_path']       = './assets/images/produk/';
    $config['allowed_types']     = 'gif|jpg|png|jpeg';
    $config['max_size']          = 4000;   //Tergantung dari php.ini juga
  ...
  public function uploadFotoProdukTambahanUpdate()
  {
    $config['upload_path']       = './assets/images/produk/';
    $config['allowed_types']     = 'gif|jpg|png|jpeg';
    $config['max_size']          = 4000;   //Tergantung dari php.ini juga
  ...
  public function data_product()
  {
    ...
    foreach ($res as $produk) {
      ...
      $newAr[$produk['id_produk']] = [
        ...
        'gambar_tambahan' => $produk['gambar_tambahan'],
  ...
  {
    if ($id != null) {
      $dataProduk = $this->Admin_Model->getDetailProduk(decrypt_url($id));
      if ($this->Admin_Model->deleteProduct(decrypt_url($id))) {
        if (file_exists('./assets/images/produk/' . $dataProduk['gambar_produk'])) {
          unlink('./assets/images/produk/' . $dataProduk['gambar_produk']);
        }
        if (file_exists('./assets/images/produk/' . $dataProduk['gambar_tambahan'])) {
          unlink('./assets/images/produk/' . $dataProduk['gambar_tambahan']);
        }
  ...
  {
    //  	dd($this->input->post('tag', true));
    $ukuran = $this->input->post('ukuran', true);
    $newUk = '';
    foreach ($ukuran as $uk) {
      $newUk .= $uk . ', ';
    }
    $unique = clean($this->input->post('id_unik', true));
    if (clean($this->input->post('id_produk', true)) == null) {
      $id = $this->Admin_Model->getIdProdukByUniqueId($unique);
    } else {
      $id = clean($this->input->post('id_produk', true));
    }

    $dataProduk = $this->Admin_Model->getDetailProduk($id);

    if (empty($_FILES['foto']['tmp_name']) || !is_uploaded_file($_FILES['foto']['tmp_name'])) {
      $fotoUtama = $dataProduk['gambar_produk'];
    } else {
      $fotoUtama = $this->uploadFotoProdukUpdate();
    }

    if (empty($_FILES['foto1']['tmp_name']) || !is_uploaded_file($_FILES['foto1']['tmp_name'])) {
      $fotoTambahan = $dataProduk['gambar_tambahan'];
    } else {
      $fotoTambahan = $this->uploadFotoProdukTambahanUpdate();
    }

    $produk = [
      'nama_produk' => clean($this->input->post('nama', true)),
      'harga_produk' => clean($this->input->post('harga', true)),
      'ket_produk' => clean($this->input->post('keterangan', true)),
      'gambar_produk' => $fotoUtama,
      'gambar_tambahan' => $fotoTambahan,
      'id_cat' => clean($this->input->post('kategori', true)),
    ];
    $detail = [
    ...
    if ($this->Admin_Model->updateProduk($produk, $detail, $newTag, $id, $unique)) {
      ...
      if ($fotoUtama != $dataProduk['gambar_produk']) {
        if (file_exists('./assets/images/produk/' . $dataProduk['gambar_produk'])) {
          unlink('./assets/images/produk/' . $dataProduk['gambar_produk']);
        }
      }
  ...
  public function contact_us()
  {
    ($this->session->level == 'admin') ? redirect('dashboard/admin/customers') : '';
    $data['title'] = 'Data Admin - Owner';
    $data['admin'] = $this->Admin_Model->getAllContact();

    $this->load->view('admin/customers/data_about', $data);
  }
