...
class Home extends CI_Controller
{
	...
	{
		$data['title'] = 'FRMT.CO - About';
		$this->load->view('about');
	}

	public function contact()
	{
		$data['title'] = 'FRMT.CO - Contact';
		if ($this->input->post('nama', true) != null) {
			$nama = $this->input->post('nama', true);
			$email = $this->input->post('email', true);
			$pesan = $this->input->post('pesan', true);
			$waktu = time();

			$insert = $this->Home_Model->tes_about($nama, $email, $pesan, $waktu);
			$this->freeM->getSweetAlertHref('message', 'Berhasil!', 'Berhasil Mengirim data', 'success');
			redirect('contact');
		}
