...
class Admin_Model extends CI_Model
{
    ...
    {
        $tes = $this->db->query("SELECT * FROM tb_about ORDER BY id_about DESC");
    ...
    public function getIdProdukByUniqueId($unique)
		{
			$data = $this->db->get_where('produk', ['unik_produk' => $unique])->row_array();
    ...
    public function updateProduk($produk, $detail, $tag, $id, $id_unik)
    {
        $this->db->trans_begin();
				if($id == null){
					$dataProduk = $this->db->get_where('produk', ['unik_produk' => $id_unik])->row_array();
